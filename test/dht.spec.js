...
describe('DHT', () => {
  it('Connect to a running node', async () => {
    ...
    const secretSocket = node.connect(DHT_KEY)

    expect(secretSocket.remotePublicKey).to.equal(DHT_KEY)
    node.destroy()
  })
  ...
  it('Create DHT node and accept a connection from another', async () => {
    ...
    const secretSocket = node2.connect(server.address().publicKey)

    expect(secretSocket.remotePublicKey).to.eql(server.address().publicKey)
